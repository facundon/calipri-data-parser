...
const EditNumberCell: React.ElementType = ({ rowData, dataKey, onChange, onPressEnter, ...props }: {rowData: Dimension, dataKey:DataKey, onChange: any, onPressEnter: any}) => {
  const { Cell } = Table
  const editing = rowData!.status === "EDIT"
  return (
    <Cell {...props} className={editing ? "table-content-number-editing" : ""}>
      {editing
        ? (
          <InputWithButtons
